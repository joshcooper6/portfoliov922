...
export default function ProjectThumb(props) {
    ...
    return(<>
            <div onAnimationEnd={() => {setClicked(false)}} className={`min-h-40 h-max lg:h-80 md:h-60 md:w-80 w-screen flex rounded-xl snap-start flex-col align-center relative justify-center text-center bg-black hover:cursor-pointer  transition-ease`}>
              ...
              <a href={SOURCE} target="_blank" rel='noreferrer' onClick={() => setClicked(true)} className={`rounded-xl absolute my-0 mx-auto w-full h-full flex flex-col justify-center text-3xl text-green-100 bg-black bg-opacity-90 hover:bg-opacity-80 transition-ease`}>
                    ...
                    <p className={`transition-ease ${TRACKER ? '' : 'opacity-0'} text-xs pb-5 pt-2 w-10/12 self-center tracking-widest text-left`}>{DESC}</p> 
              </a>
            </div>
    </>)
}